 ...
 *
 * \todo allow to register i/o filters to be executed at read/write time
 *
 */
...
const unsigned char *
rbb_dump(rbb_t *rb) {
    int data_size = rbb_len(rb);
    if(data_size > 0) {
        u_char buffer[data_size];
        if(rb->wfx < rb->rfx) {
            int to_end = rb->size - rb->rfx;
            memcpy(buffer, &rb->buf[rb->rfx], to_end);
            memcpy(&buffer[to_end], rb->buf, data_size - to_end);
        } else {
            memcpy(buffer, &rb->buf[rb->rfx], data_size);
        }
        printf("%s\n", HexEscape((char *)buffer, data_size));
    }
    return NULL;
}
#endif
